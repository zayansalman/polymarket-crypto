...
| `logging_setup.py` | WIRED | 10 | Structured JSON logging with structlog. Module + trade_id context. |
| `main.py` | cli | 0 | Entrypoint for the BTC 5-minute paper trading system. |
| `polymarket_bot/__init__.py` | pkg | 14 | BTC 5-minute paper-trading package. |
...
| `polymarket_bot/params.py` | WIRED | 4 | Active strategy parameters (Layer 2 — operator-gated auto-tune). |
...
| `polymarket_bot/runtime_knobs.py` | WIRED | 6 | Operator runtime knobs: single dashboard-editable source of truth (#206). |
...
| `polymarket_exec/ops/dashboard/panels/ribbon.py` | WIRED | 1 | Top status ribbon: mode/state pills, daily PnL split, feed liveness chips. |
