...
- **One executor per tick.** A tick reads the live executor once and its entry step uses only that one. If the runner changed mid-tick (for example an old paper runner waking after a switch to LIVE), the entry step writes nothing and posts nothing. (Found by Claude session "Tsinghua base Kronos btc 24h", 2026-09-17.)
