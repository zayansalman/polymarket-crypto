...
- **At most one order attempt per hour per strategy** (Claude, 2026-09-15, branch-review finding hourly-ambiguous-post-error-retried).
  ...
  - Exception: if the entry actually went through and only the `ENTERED` write was lost (a failed database write, or a restart after which boot reconciliation adopted the fill), the next tick records `ENTERED` with that position, even after the entry deadline. In live this needs the strategy's position slot to hold the entry; otherwise the hour still ends as `UNCERTAIN`. Positions closed by boot reconciliation (`RECONCILED_*`) never count. (Claude, 2026-09-15, branch-review finding crash-after-entry-marks-missed.)
