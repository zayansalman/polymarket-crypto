...
> Amended when implemented (Claude, 2026-09-16): the book table is unique on `(window_start_ts, offset_s)`, not the slug (branch-review finding dst-fallback-slug-collision), and decision records are per mode, so the candle audit (Task 10) looks rows up by `window_start_ts`, strategy and mode. The strategy module is `btcusdt_1h_spot_taker_push_reversal.py`.
...
- `async maybe_record(client, *, snapshot, market: HourMarket, now: int) -> bool` — writes at most one row per (hour, offset); returns True iff a row was written. Never raises: any exception is logged as `hourly_book_record.failed` and returns False.
  - elapsed = `now - market.window_start_ts`; offset = `offset_for(elapsed)`; skip if None or a row already exists (use `INSERT OR IGNORE` on the `(window_start_ts, offset_s)` unique index — keyed by the UTC hour start per branch-review finding dst-fallback-slug-collision, plus an in-memory `set[(start, offset)]` to avoid refetching).
