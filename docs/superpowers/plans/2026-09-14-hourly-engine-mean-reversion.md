...
- Produces:
  ...
  - `async finalize_pending_past_deadline(now: int, deadline_s: int) -> int` (every `PENDING` row with `window_start_ts + deadline_s < now`, any hour and any strategy: `ENTERED` with its position when this strategy has a 1h position row for that hour that boot reconciliation did not close as never placed or unfilled (`RECONCILED_NO_LIVE_TRACE` or `RECONCILED_UNFILLED`), otherwise `MISSED`; returns rows changed) _(Claude, 2026-09-15, branch-review finding pending-row-never-finalized)_
