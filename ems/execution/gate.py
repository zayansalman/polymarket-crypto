...
2. ``loss_halt``: the day's settled P&L on this leg is at or below minus the loss halt.
3. ``bad_notional``: the order is worth nothing, or not a number.
4. ``max_trade``: the order's notional (price x shares) is above the per-trade cap.
5. ``daily_cap``: today's net notional placed on this leg, plus this order, is above the cap.
...
often they are reported. Paper and live never share a total.
"""
...
MAX_TRADE = "max_trade"
DAILY_CAP = "daily_cap"
REASONS = (KILL, LOSS_HALT, BAD_NOTIONAL, MAX_TRADE, DAILY_CAP)
...
class RiskGate:
    ...
    async def check(self, notional_usd: float, *, now: float) -> Verdict:
        ...
        if halt > 0 and today.pnl_usd <= -halt:
        ...
        if notional_usd > limits.max_trade_usd + 1e-9:
        ...
        if cap > 0 and today.notional_usd + notional_usd > cap + 1e-9:
