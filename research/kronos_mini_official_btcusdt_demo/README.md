...
Wider check (`scripts/compare_recreated_with_published.py`, output `outputs/recreated_vs_published.txt`): 279 hours spread over the whole Kronos-mini era, one every 25 hours. Inputs were identical for all 279. Correlation with the published numbers was 0.897, the mean gap 0.000 and the mean absolute gap 0.085. 13 of 279 gaps (4.7%) were beyond two sampling standard errors, which is what sampling alone predicts (about 5%). The recreation reproduces the published forecast. The median run was 8.8 s of CPU on the operator's M2.
