...
| Status | maths validated against simulation and the literature (section 9); historical test pre-registered (section 8), fitted and run 2026-09-21 after two adversarial reviews: results, deviations and open findings in section 11; settlement corrected 2026-09-22 (the 15m market settles on an average price, section 1b; nothing refitted yet); never traded |
| Checks | `tools/fade_1h_momentum_15m/validate_math.py`; `twap_proxy_agreement.py` (section 1b) |
...
*Correction, 2026-09-22 (Zayan): the 15m market settles on a Chainlink TWAP, not on the close.
Sections 1 and 5 price the close and are kept for comparison; the average-price version, and
which reading of the rule the tape supports, is section 1b.*
...
## 1b. Average-price settlement

*Added 2026-09-22, after Zayan's correction. Checks: `validate_math.py` section 9 and
`twap_proxy_agreement.py`. Code: `model.py`, `prob_up_twap` and the switch `window_prob_up`.*

**The rule.** The 15m market's resolution text: it resolves Up "if the time-weighted average
price (TWAP) of Bitcoin, generated by Chainlink, of the time range specified in the title is
greater than or equal to the price at the beginning of that range. Otherwise Down." The source
is the Chainlink BTC/USD TWAP-60s data stream. So a window settles on an **average**, not on
the close. Sections 1 and 5 priced $P(\text{close} \ge \text{open})$, the wrong target. The
hourly market still settles on the Binance 1h candle's close against its open, so section 2
(reading the drift out of the 1h price) does not change.

The text can be read two ways, and the maths below covers both:

- **Whole window:** the average price over all 15 minutes, against the stream's value at the
  open.
- **Trailing minute:** the stream's own value at the close (the average over the last 60 s),
  against its value at the open.

In both, the start reference is the stream's value at the open: the average over the 60 s
**before** the open, not the open price. The tape (1b.11) agrees with the trailing-minute
reading on 96.8% of markets and with the whole-window reading on 85.2%.

### 1b.1 Setup

The window is $[t_k, t_k + T]$ with $T = \tfrac14$ hour. The decision is at $t = t_k + \tau T$
($\tau$ the fraction gone, as in the notation above), with $h = (1-\tau)T$ hours left, so the
window ends at $t + h$. The average runs over the last $\delta$ hours of the window,
$[t+h-\delta,\ t+h]$: $\delta = T$ for the whole window, $\delta = \tfrac1{60}$ for the trailing
minute. Known at the decision:

| symbol | meaning |
|---|---|
| $x_0$ | log of the start reference (the TWAP-60s value at the open) |
| $d = X(t) - x_0$ | where the price is now, measured from the start reference |
| $\bar a = \frac1\epsilon\int_{t+h-\delta}^{t} X\,ds - x_0$ | the average so far, measured from $x_0$ (only when some of the average has passed) |
| $\epsilon = \max(0,\ \delta - h)$ | hours of the average already gone |
| $\ell = \min(h, \delta)$ | hours of the average still to come |
| $t_\delta = t + h - \ell$ | where the unknown part of the average starts ($= t$ for the whole window) |

For the whole window, $\epsilon = \tau T$ and $\ell = h$.

**Log approximation.** The market averages the price $S$; the maths averages the log price $X$.
Write $\bar X$ for the average of $X$ over the averaging window. Then
$\ln(\text{average of } S) = \bar X + c$ with $c \approx \tfrac12 \times$ (the variance of $X$
around $\bar X$ inside the window) $\ge 0$ (Jensen). The log rule $\bar X \ge x_0$ says Down
on the few paths with $-c \le \bar X - x_0 < 0$, which the market settles Up. For Brownian
motion over the whole window, from the open, given $\bar X$ sits at the start reference, that
variance averages $\sigma^2\delta/10$ (not $\sigma^2\delta/6$: paths whose average ends where they
started are calmer), so $c \approx \sigma^2\delta/20$. For the trailing minute, which starts 14
minutes later, the variance is close to $\sigma^2\delta/6$ and $c$ to $\sigma^2\delta/12$. The error in $p$ is about $c$ times the density
of $\bar X - x_0$ at 0. The last two columns are the worst case late in the window, when the
average sits right at the start reference:

| $\sigma$ per $\sqrt{\text{hour}}$ | whole window, at the open | trailing minute, at the open | whole window, 1 min left, at most | trailing minute, 30 s left, at most |
|---|---|---|---|---|
| 0.3% | 0.005 points | 0.0003 | 0.50 | 0.006 |
| 0.5% | 0.009 | 0.0006 | 0.84 | 0.011 |
| 1.0% | 0.017 | 0.001 | 1.67 | 0.021 |
| 2.0% | 0.035 | 0.002 | 3.35 | 0.042 |

(percentage points of probability). Simulated on the same price paths, the two rules differ on
27 of 200,000 paths against 17.3 expected at 0.5% (z +2.34), and 57 against 69.1 at 2% (z −1.46);
0 and 1 paths for the trailing minute. At the decision minutes used so far (0 to 5) and
$\sigma = 0.5\%$ the error is at most 0.03 points for the whole window and far less for the
trailing minute; it scales with $\sigma$. It becomes material only in the last minute or two of
a whole-window average, where the part of $c$ already realised is known and could be added to
the threshold; that is not implemented.

### 1b.2 Split what is known from what is not

Average over the window, times $\delta$, minus $\delta x_0$:

$$\delta(\bar X - x_0) = \epsilon\,\bar a + \int_{t_\delta}^{t+h} (X(s) - x_0)\,ds
= \epsilon\,\bar a + \ell\, d + I, \qquad I = \int_{t_\delta}^{t+h} D(s)\,ds,\quad D(s) = X(s) - X(t).$$

So the window resolves Up iff $\epsilon\,\bar a + \ell\, d + I \ge 0$. The first two terms are known.
$I$ is Gaussian under section 1's process, so everything reduces to its mean and variance.

For the whole window: $\tau T\,(\bar A - x_0) + h\,(X(t) - x_0) + I \ge 0$, with
$I = \int_t^{t+h} D(s)\,ds$.

### 1b.3 The mean of $I$

Section 1 gives the mean of $D(s)$ for any $s > t$ (its $E[R]$ with $h$ replaced by $s - t$):
$m(s) = -\big(1 - e^{-K(t,s)}\big) M_t + \mu\, G(t,s)$, with
$G(t,s) = \int_t^s e^{-K(u,s)}du$. Define the **forward** integral

$$L(u) = \int_u^{t+h} e^{-K(u,s)}\,ds,$$

the time-integral, over the rest of the window, of what is left on average of a unit push at
$u$. Section 1's $G$ integrates over the start with the end fixed; $L$ integrates over
the end with the start fixed. Whole window first ($t_\delta = t$):

$$E[I] = \int_t^{t+h} m(s)\,ds = -B\,M_t + \mu\,\bar G,$$
$$B = \int_t^{t+h}\big(1 - e^{-K(t,s)}\big)ds = h - L(t), \qquad
\bar G = \int_t^{t+h} G(t,s)\,ds = \int_t^{t+h} L(u)\,du .$$

The second form of $\bar G$ swaps the order of integration over the triangle $t \le u \le s \le t+h$.
$\bar G$ is the time-integral of section 1's $G$: the momentum's route into the average.

### 1b.4 The variance of $I$

The part of $D$ that is not its mean solves $dZ = -\kappa(s) Z\,ds + \sigma\,dW$, $Z(t) = 0$, so
$Z(s) = \sigma\int_t^s e^{-K(u,s)}dW(u)$. Two ways to the variance:

*As a double integral.* For $s_1 \le s_2$, $\mathrm{Cov}(D(s_1), D(s_2)) = e^{-K(s_1,s_2)}\,V(t,s_1)$,
with $V(t,s_1) = \sigma^2\int_t^{s_1} e^{-2K(u,s_1)}du$ section 1's variance. Then
$\mathrm{Var}[I] = 2\iint_{t \le s_1 \le s_2 \le t+h} e^{-K(s_1,s_2)}\,V(t,s_1)\,ds_1\,ds_2$, a triple
integral once $V$ is written out.

*Reduced to one dimension.* Swap the $ds$ and $dW$ integrals (stochastic Fubini):
$\int_t^{t+h} Z(s)\,ds = \sigma\int_t^{t+h}\Big[\int_u^{t+h} e^{-K(u,s)}ds\Big]dW(u) = \sigma\int_t^{t+h} L(u)\,dW(u)$.
By the Itô isometry

$$\mathrm{Var}[I] = \sigma^2\,\Psi, \qquad \Psi = \int_t^{t+h} L(u)^2\,du .$$

A shock at time $u$ enters the average with weight $L(u)$, and the variance adds up the squared
weights. The unit tests check this against the double integral (relative 1e-7).

### 1b.5 Closed forms and quadrature

**Brownian** ($\kappa_0 = 0$): $L(u) = t + h - u$, so $B = 0$, $\bar G = h^2/2$, $\Psi = h^3/3$:

$$E[I] = \mu h^2/2, \qquad \mathrm{Var}[I] = \sigma^2 h^3/3 .$$

**Constant pull** ($\lambda = 0$): $L(u) = (1 - e^{-\kappa(t+h-u)})/\kappa$, the integrated OU process:

$$B = h - \frac{1-e^{-\kappa h}}{\kappa}, \qquad \bar G = \frac{B}{\kappa}, \qquad
\Psi = \frac1{\kappa^2}\Big[h - \frac{2(1-e^{-\kappa h})}{\kappa} + \frac{1-e^{-2\kappa h}}{2\kappa}\Big],$$

so $E[I] = (\mu/\kappa - M_t)\,B$: the price reverts toward a level $\mu/\kappa$ above the anchor.

**Decaying (or growing) pull** ($\kappa(s) = \kappa_0 e^{-\lambda s}$, $\lambda$ of either sign). $L$ has a
closed form. With $A = \kappa_0/\lambda$ and $w_s = e^{-\lambda s}$, $K(u,s) = A(w_u - w_s)$ and
$ds = -dw/(\lambda w)$, so

$$L(u) = \frac{e^{-A w_u}}{\lambda}\Big[\mathrm{Ei}(A w_u) - \mathrm{Ei}(A w_{t+h})\Big],$$

$\mathrm{Ei}$ the exponential integral (for $\lambda < 0$, $A w < 0$ and $\mathrm{Ei}(x) = -E_1(-x)$; the
formula holds as written). Equivalently, reversing time ($s \to -s$) turns $\kappa_0 e^{-\lambda s}$
into $\kappa_0 e^{+\lambda s}$ and swaps start and end, so $L(u)$ is section 1's $G$ on
$[-(t+h), -u]$ with $\lambda \to -\lambda$. The code uses that identity, which reuses section 1's
exact routine. $\bar G$ and $\Psi$ are integrals of $L$ and $L^2$ and have no elementary closed
form, so they are one-dimensional integrals of the closed-form $L$, taken by 64-point
Gauss–Legendre on $[t_\delta, t+h]$ (the double integral of exponentials is reduced to one
dimension; nothing is nested numerically). Against nested adaptive quadrature the relative
error is about 1e-12 or smaller. $B = h - L(t)$ is exact; where the pull is weak ($K(t,t+h) < \tfrac12$) the
subtraction cancels, so there $B$ is integrated directly, $\int(1 - e^{-K(t,s)})ds$.

### 1b.6 The probability, whole window

$$\boxed{\;p = \Phi\!\left(\frac{\tau T\,(\bar A - x_0) + h\,(X(t) - x_0) - B\,M_t + \theta\hat\mu\,\bar G}{\sqrt{\sigma^2\Psi + \theta^2\hat v\,\bar G^2}}\right)\;}$$

$\theta$, the blend $(\hat\mu, \hat v)$ of section 4 and the 1h inversion of section 2 are unchanged.
The drift is uncertain with variance $\theta^2\hat v$ and enters $I$ through $\bar G$, which is where
the $\theta^2\hat v\,\bar G^2$ comes from.

**The weights, in words.** Divide the numerator by $T$ to read it as a contribution to the average:

- The average so far is locked in, with weight $\tau$: the share of the window already gone.
- Where the price is now counts with weight $h/T = 1 - \tau$: it stays there, on average, for the
  rest of the window.
- A move $r$ hours before the end shifts the price for those $r$ hours only, so it counts $r/T$.
  Late moves count least; a move in the last seconds barely counts.

Consequences at the open: the future's variance in the average is $\sigma^2 T/3$, a third of the
close's ($\sigma^2 T$), so the average is harder to move. The drift's effect is $\mu T/2$ instead of
$\mu T$. Per unit of noise the drift counts $\sqrt3/2 = 0.87$ times what it counts for the close.

### 1b.7 Any averaging window, and the trailing minute

For $\delta < T$ and a decision before the average starts ($t_\delta > t$), a shock at $u < t_\delta$
reaches the average with weight $\int_{t_\delta}^{t+h} e^{-K(u,s)}ds = e^{-K(u,t_\delta)}L(t_\delta)$,
and one at $u \ge t_\delta$ with weight $L(u)$. Splitting the same integrals at $t_\delta$, and writing
$K$, $G$, $V$ for section 1's over $[t, t_\delta]$:

$$B = \ell - e^{-K(t,t_\delta)}L(t_\delta), \qquad
\bar G = G(t,t_\delta)\,L(t_\delta) + \int_{t_\delta}^{t+h}L(u)\,du, \qquad
\Psi = \frac{V(t,t_\delta)}{\sigma^2}\,L(t_\delta)^2 + \int_{t_\delta}^{t+h}L(u)^2\,du,$$

$$p = \Phi\!\left(\frac{\epsilon\,\bar a + \ell\, d - B\,M_t + \theta\hat\mu\,\bar G}{\sqrt{\sigma^2\Psi + \theta^2\hat v\,\bar G^2}}\right).$$

- $\delta = T$: $t_\delta = t$, and this is 1b.6.
- Brownian: $\bar G = g\ell + \ell^2/2$ and $\Psi = g\ell^2 + \ell^3/3$, with $g = t_\delta - t$.
- $\delta \to 0$: $B/\ell \to 1 - e^{-K}$, $\bar G/\ell \to G$, $\Psi/\ell^2 \to V/\sigma^2$, which is section 5 with
  $y_\tau$ replaced by $d$ (the close, measured from the start reference). Checked numerically at
  $\delta = 10^{-7}$ hours: they agree to 7 digits, the rest being the $O(\delta)$ term.
- Trailing minute ($\delta = \tfrac1{60}$): the close smoothed over its last 60 s. At the open the
  future's standard deviation is 0.978 of the close's and the drift counts 0.99 of what it
  counts for the close. The main difference from section 5 is the start reference.

### 1b.8 The start reference

The start reference is the TWAP-60s value at the open, roughly the average over the minute
before it, not the price at the open. For Brownian motion with drift over that minute
($\delta_0 = \tfrac1{60}$ hour), $X(t_k) - x_0 \sim N(\mu\delta_0/2,\ \sigma^2\delta_0/3)$: a standard
deviation of 3.7bp at $\sigma = 0.5\%$ (simulation: mean and variance agree, z +0.68 and −0.10).
It is known at the open, and it matters there: on a typical window (one standard deviation)
it shifts $z$ at the open by 0.26 for the whole window and 0.15 for the trailing minute. In the
simulation, with the step-1 pull ($\kappa_0$ 0.345, $\lambda$ −1.62), a drift of 0.2% an hour and a
stretch of 0.15%, $p$ at the open runs from 0.35 to 0.71 (5% to 95% of paths) for the whole
window and from 0.42 to 0.64 for the trailing minute. Ignoring the offset gives 0.52 on every
path. So $d$ and $\bar a$ must be measured from $x_0$, not from the open. Steps 0 to 2
measured $y_\tau$ from the Binance open.

### 1b.9 What changes in section 5's worked cases

Momentum isolated ($\theta = 1$, $\kappa_0 = 0$), $\sigma = 0.5\%$, $x_0$ = the price at the open:

| case | close (section 5) | whole window | trailing minute |
|---|---|---|---|
| :00, 1h market at 60c | 0.5504 | 0.5437 | 0.5498 |
| :45, hour up 0.3%, 1h still 60c | 0.1719 | 0.2062 | 0.1746 |
| weight on the 1h price at :00 / :15 / :30 / :45 | 0.50 / 0.58 / 0.71 / 1.00 | 0.43 / 0.50 / 0.61 / 0.87 | 0.49 / 0.57 / 0.70 / 0.99 |

For the whole window the weight is $\tfrac{\sqrt3}{4}/\sqrt{1-t}$, exactly $\sqrt3/2$ of the close's.

### 1b.10 Checked by simulation

`validate_math.py` section 9, seed 20260922, 200,000 paths per case. Each path runs section 1's
process from the decision with exact OU steps (pull taken at mid-step), 200 steps before the
averaging window and 400 inside it, and the average is the trapezoid rule on those steps. The
closed forms and the quadrature come from a separate implementation (the Ei form of $L$), and
the unit tests tie it to `model.py`. z = (simulation − formula) / Monte Carlo standard error.

| case (drift ~ N(μ, v)) | E[I] z | Var[I] z | P(Up) formula | simulated | z |
|---|---|---|---|---|---|
| Brownian, whole window, open | +0.03 | +0.58 | 0.6862 | 0.6863 | +0.18 |
| constant pull (κ 3), whole window, minute 2 | +1.24 | −0.11 | 0.5518 | 0.5520 | +0.18 |
| decaying pull (κ₀ 3, λ 4), whole window, open | −0.65 | +1.01 | 0.3387 | 0.3380 | −0.68 |
| growing pull (κ₀ 0.8, λ −1.6), whole window, minute 10, v > 0 | −0.77 | +0.69 | 0.8019 | 0.8011 | −0.91 |
| strong pull (κ₀ 12, λ 0.5), whole window, minute 1, v > 0 | −1.15 | −0.60 | 0.9733 | 0.9734 | +0.19 |
| decaying pull, trailing minute, minute 2 | −1.49 | −1.88 | 0.5142 | 0.5121 | −1.83 |
| growing pull, trailing minute, 30 s left (part of the average known), v > 0 | −0.50 | +0.28 | 0.5337 | 0.5337 | −0.01 |
| step-1 fit (κ₀ 0.345, λ −1.62), trailing minute, open, v > 0 | +1.53 | +0.37 | 0.4621 | 0.4648 | +2.42 |

The last case rerun with 800,000 paths and another seed: z −0.67, −0.15, −1.16. The start
reference (1b.8): the offset's mean and variance agree (z +0.68, −0.10); the formula's $p$ given
each path's offset averages to the simulated Up rate (whole window z −0.44, trailing minute z
+2.43), and in ten bins of $p$ the largest |z| is 1.75 and 2.14. The largest |z| over all 52
simulation checks is 2.43, in line with Monte Carlo noise across that many checks.

### 1b.11 Which reading the tape supports

`twap_proxy_agreement.py`: every settled 15m market in `m15.db` (1,136, BTC/ETH/SOL/XRP, Sep
17–20; real resolutions from the winner column), against Binance 1-minute candles. A kline
opening at $s$ has $o$ = price at $s$ and $c$ = price at $s + 60$, so $(o + c)/2$ stands for the
average over that minute. Proxies, each Up iff end ≥ start, ties Up:

- **close vs open**: $c$ of the last minute ≥ $o$ of the first (what steps 0–2 used);
- **whole-window average**: the mean over the 15 window minutes of $(o+c)/2$ ≥ $(o+c)/2$ of the
  minute before the open (the proxy of the six-month quick check);
- **trailing minute**: $(o+c)/2$ of the last minute ≥ $(o+c)/2$ of the minute before the open.

| proxy | agrees | rate (Wilson 95%) | against close vs open: right only here / right only there | difference (t, clustered by window, 284 windows) |
|---|---|---|---|---|
| close vs open | 1,078 / 1,136 | 0.949 (0.935–0.960) | — | — |
| **whole-window average** | 968 / 1,136 | **0.852** (0.830–0.872) | 43 / 153 | −0.097 (−6.70) |
| **trailing minute** | 1,100 / 1,136 | **0.968** (0.956–0.977) | 37 / 15 | +0.019 (+2.79) |
| average of the last 2 minutes | 1,081 | 0.952 | 37 / 34 | +0.003 (+0.31) |
| average of the last 3 minutes | 1,058 | 0.931 | 39 / 59 | −0.018 (−1.78) |
| average of the last 5 minutes | 1,039 | 0.915 | 38 / 77 | −0.034 (−3.09) |
| average of the last 10 minutes | 978 | 0.861 | 37 / 137 | −0.088 (−6.42) |
| whole window, $(o+h+l+c)/4$ | 970 | 0.854 | 45 / 153 | −0.095 (−6.61) |
| trailing minute, $(o+h+l+c)/4$ | 1,106 | 0.974 | 39 / 11 | +0.025 (+3.58) |
| whole window, start = open price | 969 | 0.853 | 38 / 147 | −0.096 (−6.72) |
| trailing minute, start = open price | 1,089 | 0.959 | 21 / 10 | +0.010 (+1.82) |

By coin, trailing minute: BTC 273/284, ETH 275/284, SOL 278/284, XRP 274/284; whole window: BTC
231, ETH 251, SOL 239, XRP 247 (of 284 each).

Agreement falls steadily as the averaging window lengthens past one minute, and the minute before
the open beats the open price as the start. The tape fits the trailing-minute reading: the
TWAP-60s value at the close against its value at the open. It does not fit a 15-minute
average. Binance is not Chainlink, so no proxy reaches 100%. The tape covers Sep 17–20 only; if
the rule changed after that, this table does not see it.

What this means for work already done:

- The six-month quick check of 2026-09-22 (the last window's average went the hour's way 43.6–46.1%
  of the time) used the whole-window proxy, which matches the real result on 85.2% of tape
  markets. It needs rerunning with the trailing-minute proxy before it is read.
- Steps 0–2 used close vs open (94.9%). Step 2 scored against the real resolutions, but fed the
  model $y_\tau$ from the Binance open rather than $d$ from the start reference (1b.8).

### 1b.12 In the code

`model.py`: `twap_moments` ($B$, $\bar G$, $\mathrm{Var}[I]$), `prob_up_twap`, `martingale_prob_up_twap`
(the no-edge baseline, $\kappa_0 = \theta = 0$), and the one switch `window_prob_up(...,
settlement="twap" | "close", abar=, avg_len=)` with `martingale_window_prob_up` beside it.
`avg_len` is $\delta$: `Q` (whole window) or `AVG_60S` (trailing minute). The likelihood takes the
same switch (`neg_log_lik`, `neg_log_lik_vol_by_quarter`, `fit_mle`, `settlement=`, `avg_len=`)
with analytic gradients; its default stays "close", what every fit so far used. The old
endpoint functions are unchanged, for comparison. Nothing has been refitted or rerun.

### 1b.13 Deviations and open points

1. The task set out the whole-window average. The maths is written for any averaging window
   $\delta$ (1b.7) because the tape favours the trailing minute (1b.11). The code's default for
   `avg_len` is the whole window, as specified; the choice is left to Zayan.
2. The log approximation is kept, with its error stated (1b.1). The realised part of the
   Jensen term is not added, which matters only in the last minute or two of a whole-window
   average.
3. The start reference is modelled as the plain average over the 60 s before the open. How
   Chainlink computes the TWAP-60s value (sampling, weighting) is not known here. On the tape,
   Binance stands in for Chainlink, and $(o+c)/2$ is a two-point stand-in for a 60-s average.
4. The likelihood also takes the settlement switch (not asked for), so a refit can use it.
5. The crowd model behind the maker fill (section 6, `crowd_boundary`, `maker_fill`) still
   prices the close. It is not changed here.
6. The Monte Carlo reference computes $B$ as written, $\ell - e^{-K}L(t_\delta)$, which cancels
   under a very weak pull (relative error up to 5e-9 when $B \approx 10^{-6}\,\ell$); the unit test
   compares $B$ on the scale of $\ell$ there. `model.py` integrates $B$ directly in that case.
