...
def _order(slug: str, side: str = "Up", *, kind: str = "entry", price: float = 0.40,
           shares: float = 10.0, rung: int = 0, depth_ahead: float = 0.0) -> NewOrder:
    ...
    return NewOrder(window_slug=slug, token_id=token, side=side, kind=kind, price=price,
                    shares=shares, rung=rung, depth_ahead=depth_ahead)
...
CORE = {
    ...
    "fade_orders": {"id", "window_slug", "token_id", "side", "kind", "rung", "price",
...
async def test_old_tables_gain_new_columns(tmp_path: Path, monkeypatch) -> None:
    ...
    async with aiosqlite.connect(path) as conn:
        await conn.executescript(
            ...
            CREATE TABLE fade_orders (
              ...
              rung INTEGER NOT NULL DEFAULT 0, price REAL NOT NULL, shares REAL NOT NULL,
              state TEXT NOT NULL DEFAULT 'resting', placed_ts INTEGER NOT NULL);
            INSERT INTO fade_windows VALUES ('w', 'btc', 900, 1800, 0, NULL, 1);
            INSERT INTO fade_orders (window_slug, token_id, side, kind, price, shares,
                                     placed_ts) VALUES ('w', 't', 'Up', 'entry', 0.4, 10, 1000);
            """
        )
        await conn.commit()
    ...
    assert set(_db.FADE_ORDER_COLUMN_MIGRATIONS) <= await _columns("fade_orders")
...
async def test_latest_decision_per_coin_with_inputs_round_trip(fade_db) -> None:
    for i, asset in enumerate(["sol", "btc", "sol", "btc", "eth"]):
        await ledger.record_decision(
            ...
            ladder=[{"rung": 0, "price": 0.49, "shares": 4.0}],
            hedge={"side": "Down", "price": 0.45, "shares": 0.0},
    ...
    assert btc["ladder"] == [{"rung": 0, "price": 0.49, "shares": 4.0}]
    assert btc["hedge"]["side"] == "Down"
    assert btc["factors"]["momentum"] == 0.1
    assert "inputs_json" not in btc
...
async def test_decision_with_nothing_placed(fade_db) -> None:
    ...
    assert (d["side"], d["p"], d["ladder"], d["hedge"]) == (None, None, [], None)
...
async def test_place_and_list_open_orders(fade_db) -> None:
    slug = await _window()
    ids = await ledger.place_orders(
        [_order(slug, price=0.45, rung=0, depth_ahead=120.0), _order(slug, price=0.40, rung=1)],
        ts=Q2 + 60,
    )
    ...
    assert await ledger.place_orders([_order(slug, price=0.45, rung=0)], ts=Q2 + 60) == [None]
...
async def test_new_order_checks() -> None:
    for kwargs in ({"price": 0.0}, {"price": 1.0}, {"price": math.nan}, {"shares": 0.0},
                   {"side": "up"}, {"kind": "taker"}, {"depth_ahead": -1.0}, {"rung": -1}):
...
async def test_requote_cancels_and_places_in_one_step(fade_db) -> None:
    slug = await _window()
    (old,) = await ledger.place_orders([_order(slug, price=0.45)], ts=Q2 + 60)
    (new,) = await ledger.place_orders([_order(slug, price=0.43)], ts=Q2 + 120,
                                       cancel_ids=[old])
...
async def test_expire_due_uses_the_ledgers_own_window_end(fade_db) -> None:
    ...
    (a,) = await ledger.place_orders([_order(ended)], ts=Q2 + 60)
    (b,) = await ledger.place_orders([_order(live)], ts=Q2 + 60)
...
async def test_partial_then_full_fill(fade_db) -> None:
    slug = await _window()
    (oid,) = await ledger.place_orders([_order(slug, shares=10.0)], ts=Q2 + 60)
...
async def test_bad_fills_write_nothing(fade_db) -> None:
    ...
    for bad in (
        ...
        FlowUpdate(order_id=b, cursor_ts=Q2 + 200, add_shares=1.0, fill_ts=Q2 + 30),  # before
...
async def test_orders_needing_flow(fade_db) -> None:
    slug = await _window()
    resting, cancelled, filled = await ledger.place_orders(
        [_order(slug, price=0.45), _order(slug, price=0.40, rung=1),
         _order(slug, price=0.35, rung=2, shares=2.0)], ts=Q2 + 60)
    ...
    assert (need[resting]["flow_from"], need[resting]["flow_until"]) == (Q2 + 60, Q2_END)
    assert (need[cancelled]["flow_from"], need[cancelled]["flow_until"]) == (Q2 + 60, Q2 + 120)
...
async def _hedged_window(start: int = Q2) -> tuple[str, int, int, int]:
    """An entry (10 Up at 0.40, filled), a hedge (5 Down at 0.50, filled) and a bid that
    never filled (4 Up at 0.30), with the tape read to the window end."""
    slug = await _window(start=start)
    entry, hedge, unfilled = await ledger.place_orders(
        [_order(slug, "Up", price=0.40, shares=10.0),
         _order(slug, "Down", kind="hedge", price=0.50, shares=5.0),
         _order(slug, "Up", price=0.30, shares=4.0, rung=1)], ts=start + 60)
    end = start + 900
    await ledger.record_flow([
        FlowUpdate(order_id=entry, cursor_ts=end, crossed=10.0, add_shares=10.0,
                   fill_ts=start + 100),
...
async def test_settle_a_hedged_window(fade_db) -> None:
    ...
    result = await ledger.settle_window(slug, outcome="Up", ts=Q2_END + 120)
    assert result.net_pnl == pytest.approx(10 * 0.60 - 5 * 0.50)
    assert (result.orders, result.filled_shares, result.staked_usd) == (3, 15.0, 6.5)
    assert result.forced_pending == 0

    e, h, u = [await _order_row(i) for i in (entry, hedge, unfilled)]
    assert (e["won"], e["pnl"]) == (1, pytest.approx(6.0))
    assert (h["won"], h["pnl"]) == (0, pytest.approx(-2.5))
    assert (u["state"], u["pnl"], u["cancelled_ts"]) == ("expired", 0.0, Q2_END)
    w = await ledger.get_window(slug)
    assert (w["outcome"], w["settled_ts"], w["net_pnl"]) == ("Up", Q2_END + 120,
                                                             pytest.approx(3.5))
...
async def test_settlement_waits_for_the_tape(fade_db) -> None:
    slug = await _window()
    (oid,) = await ledger.place_orders([_order(slug)], ts=Q2 + 60)
...
async def test_settle_checks(fade_db) -> None:
    ...
    (oid,) = await ledger.place_orders([_order(slug)], ts=Q2 + 1)
...
async def test_empty_summary_has_no_made_up_ratios(fade_db) -> None:
    ...
    assert (s["cents_per_share"], s["return_on_staked"], s["hedge_stake_share"]) == (
        None, None, None
    )
...
async def test_summary_profit_first(fade_db) -> None:
    # Window 1: +3.5 (hedged, Up won). Window 2: 10 Up at 0.40 lose -> -4.0.
    # Window 3: 5 Down at 0.80 win -> +1.0. Equity 3.5, -0.5, 0.5: peak 3.5, drawdown 4.0.
    ...
    (o2,) = await ledger.place_orders([_order(w2, "Up", price=0.40, shares=10.0)],
                                      ts=Q2_END + 10)
    ...
    (o3,) = await ledger.place_orders([_order(w3, "Down", price=0.80, shares=5.0)],
                                      ts=Q2_END + 910)
    await _fill(o3, 5.0, at=Q2_END + 920, cursor=Q2_END + 1800)
    await ledger.settle_window(w3, outcome="Down", ts=Q2_END + 1800)
    # Open: 3 of 6 filled on a live window, and a bid still resting there.
    w4 = await _window(asset="btc", start=Q2_END + 1800)
    (o4, o5) = await ledger.place_orders(
        [_order(w4, "Up", price=0.50, shares=6.0), _order(w4, "Up", price=0.45, shares=2.0,
                                                          rung=1)], ts=Q2_END + 1810)
    await _fill(o4, 3.0, at=Q2_END + 1820, cursor=Q2_END + 1830)
    ...
    s = await ledger.summary()
    assert s["net_pnl_usd"] == pytest.approx(0.5)
    assert s["settled_windows"] == 3
    assert s["settled_shares"] == 30.0
    assert s["staked_usd"] == pytest.approx(4.0 + 2.5 + 4.0 + 4.0)
    assert s["cents_per_share"] == pytest.approx(100 * 0.5 / 30)
    assert s["return_on_staked"] == pytest.approx(0.5 / 14.5)
    assert s["max_drawdown_usd"] == pytest.approx(4.0)
    assert s["hedge_stake_share"] == pytest.approx(2.5 / 14.5)
    assert (s["open_exposure_usd"], s["open_windows"]) == (pytest.approx(1.5), 1)
    assert (s["resting_usd"], s["resting_orders"]) == (pytest.approx(3 * 0.50 + 2 * 0.45), 2)
    assert (s["orders_placed"], s["orders_filled"]) == (7, 5)
    assert s["windows_observed"] == 4

    assert await ledger.free_cash_usd(100.0) == pytest.approx(100 + 0.5 - 1.5 - 2.4)
    (pos,) = await ledger.open_positions()
    assert (pos["window_slug"], pos["asset"], pos["side"]) == (w4, "btc", "Up")
    assert (pos["shares"], pos["cost_usd"], pos["avg_price"], pos["hedge_shares"]) == (
        3.0, pytest.approx(1.5), pytest.approx(0.5), 0
    )
