...
def armed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(_config, "POLYMARKET_SIGNATURE_TYPE", 2)  # MetaMask
...
def _token(client: TestClient) -> dict[str, str]:
    """The dashboard page's LIVE token, as the page's own JS sends it."""
...
def test_switch_to_live_succeeds_when_unarmed(client: TestClient, unarmed: None) -> None:
    r = client.post("/api/mode", json={"mode": "live"}, headers=_token(client)).json()
...
def test_unarmed_live_start_refuses_and_runs_nothing(
    ...
    client.post("/api/mode", json={"mode": "live"}, headers=_token(client))
    r = client.post("/api/start", headers=_token(client)).json()
...
def test_switch_back_to_paper(client: TestClient, unarmed: None) -> None:
    assert client.post("/api/mode", json={"mode": "live"}, headers=_token(client)).json()["mode"] == "live"
    assert 'class="mode-opt live active' in client.get("/").text
...
@pytest.mark.asyncio
async def test_live_start_refused_unless_live_clicked_in_dashboard(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, armed: None, isolated_controller: None
) -> None:
    """Env BOT_MODE=live alone is not consent — only the dashboard LIVE click is."""
    monkeypatch.setattr(_db, "DB_PATH", tmp_path / "consent.db")
    await _db.init_db()
    ...
    assert spawned == []
    assert status.state == "stopped"
    assert "not clicked" in status.detail


@pytest.mark.asyncio
async def test_live_start_proceeds_after_live_click_without_env_phrase(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, armed: None, isolated_controller: None
) -> None:
    monkeypatch.setattr(_db, "DB_PATH", tmp_path / "consent_ok.db")
    await _db.init_db()
    ...
    await controller.set_mode("live")  # the operator's click
    status = await controller.request_start()

    assert spawned == [False]
    assert status.state == "running"
    assert controller._mode_cache == "live"


def test_live_selection_without_dashboard_token_refused(client: TestClient, armed: None) -> None:
    """A bare HTTP client (curl, another process, a web page) can't click LIVE."""
    r = client.post("/api/mode", json={"mode": "live"}).json()
    assert r["status"] == "error"
    assert "clicking LIVE in the dashboard" in r["detail"]
    assert controller.live_consented() is False
    bad = client.post("/api/mode", json={"mode": "live"}, headers={"X-Dashboard-Token": "nope"}).json()
    assert bad["status"] == "error"


def test_live_start_without_dashboard_token_refused(
    client: TestClient, armed: None, monkeypatch: pytest.MonkeyPatch
) -> None:
    spawned: list[bool] = []
    monkeypatch.setattr(controller, "_ensure_runner_started", lambda force=False: spawned.append(force))
    monkeypatch.setattr(controller, "_ensure_watchdog_started", lambda: None)
    ...
    assert spawned == []
    assert r["status"] == "error"
    assert "pressed in the dashboard" in r["detail"]


@pytest.mark.asyncio
async def test_persisted_live_selection_is_not_consent(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, armed: None, isolated_controller: None
) -> None:
    """A LIVE row left by an earlier process (restart) never arms a new one."""
    monkeypatch.setattr(_db, "DB_PATH", tmp_path / "stale.db")
    await _db.init_db()
    await _db.set_config("polymarket_bot.requested_mode", "live")
    spawned: list[bool] = []
    monkeypatch.setattr(controller, "_ensure_runner_started", lambda force=False: spawned.append(force))
    monkeypatch.setattr(controller, "_ensure_watchdog_started", lambda: None)
...
def test_stale_live_selection_hint_asks_to_click_again(client: TestClient, armed: None) -> None:
    import asyncio

    asyncio.run(_db.set_config("polymarket_bot.requested_mode", "live"))
    button = _live_button(client.get("/").text)
    assert "click LIVE again" in button
...
def test_dashboard_js_has_no_browser_dialogs(client: TestClient) -> None:
    """Operator rule: never confirm()/alert()/prompt() pop-ups — the click is the intent."""
    import re
...
def test_unarmed_hint_is_concise(client: TestClient, unarmed: None) -> None:
    button = _live_button(client.get("/").text)
    assert "not armed: POLYMARKET_PRIVATE_KEY is not set" in button
    assert "fall back to paper" not in button
